...
export default function RestaurantsPage() {
  ...
  useEffect(() => {
    ...
    const newUrl = params.toString() ? `?${params.toString()}` : '/restaurants';
            ...
            {!loading && filteredRestaurants.length > 0 && (
              <div className={`grid gap-8 ${
                ...
                {filteredRestaurants.map((restaurant) => {
                  // Transform availability to match EnhancedRestaurantCard format
                  const availability = restaurant.availability ? {
                    availableSlots: restaurant.availability.timeSlots?.filter(slot => slot.available).length || 0,
                    nextAvailable: restaurant.availability.timeSlots?.find(slot => slot.available)?.time || '',
                    alternatives: restaurant.availability.timeSlots?.filter(slot => !slot.available).length || 0
                  } : undefined;
                    ...
                    <RestaurantCard
                      ...
                      restaurant={{
                        ...
                        menu: restaurant.menu ? {
                          ...
                          analytics: {
                            dataSource: restaurant.menu.source,
                            reliability: 0.8,
                            totalItems: restaurant.menu.categories.reduce((acc, cat) => acc + cat.items.length, 0),
                            averagePrice: restaurant.menu.categories.reduce((acc, cat) => {
                              const catAvg = cat.items.reduce((sum, item) => sum + item.price, 0) / (cat.items.length || 1);
                              return acc + catAvg;
                            }, 0) / (restaurant.menu.categories.length || 1)
                          }
                        } : undefined
                      }}
