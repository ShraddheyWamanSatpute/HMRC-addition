...
3. **Register your application** - **IMPORTANT: Application name must match your company name**
...
6. Store credentials securely (Firebase Secrets - see `functions/env.example`)

**⚠️ CRITICAL REQUIREMENTS:**
- ✅ **Only ONE production application per organization** - do not create multiple applications
- ✅ **Application name must match your company name** - this is required for compliance
...
After registering your application, you can optionally set the `HMRC_APPLICATION_NAME` environment variable in Firebase Functions to match your company name. This is used for compliance auditing:
...
```bash
export HMRC_APPLICATION_NAME="Your Company Name"
```

**Note:** The system validates that only one application is configured at runtime. This ensures HMRC compliance requirements are met.
...
### Global Root CA Keystore (Automatic)
...
**IMPORTANT:** HMRC API IP addresses are NOT static. Always use domain names for firewall rules and network configuration.
