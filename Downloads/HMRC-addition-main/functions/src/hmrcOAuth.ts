...
export const refreshHMRCToken = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      // Logs configuration for auditing (doesn't fail if name not set)
      try {
        validateSingleApplication(clientId, clientSecret);
      } catch (error: any) {
        console.error('[HMRC OAuth] Application validation failed:', error.message);
        // Validation failure means credentials not configured - already handled above
      }
