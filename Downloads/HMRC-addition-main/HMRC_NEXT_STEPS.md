...
- [ ] Store credentials securely (Firebase Secrets - see `functions/env.example`)
...
**Note:** You can use ONE master application for all companies, OR each company can register their own. See `HMRC_MULTI_TENANT_GUIDE.md` for details. **We recommend using ONE master application for simplicity and compliance.**
