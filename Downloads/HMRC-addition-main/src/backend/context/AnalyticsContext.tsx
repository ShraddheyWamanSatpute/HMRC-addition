...
export const AnalyticsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const analyzeBookings = async (startDate?: string, endDate?: string) => {
    try {
      ...
      if (startDate && endDate) {
        filtered = bookings.filter((b: any) => (b.date || '') >= startDate && (b.date || '') <= endDate);
      }
  ...
  const analyzeFinance = async (startDate?: string, endDate?: string) => {
    try {
      ...
      if (startDate && endDate) {
        filtered = transactions.filter((t: any) => (t.date || '') >= startDate && (t.date || '') <= endDate);
      }
  ...
  const analyzePOS = async (startDate?: string, endDate?: string) => {
    try {
      ...
      let filtered = bills;
      if (startDate && endDate) {
        filtered = bills.filter((b: any) => {
          const d = (b.tradingDate || b.date || '').toString();
  ...
  const analyzeMessenger = async () => {
    try {
      ...
      const messages: any[] = []; // TODO: Implement getMessages
...
Please provide specific metrics and recommendations for optimizing notification systems.`;
  ...
  const getFinanceWidgets = async (dateRange?: { startDate: string; endDate: string }): Promise<FinanceWidgets> => {
    try {
      ...
      const bankAccounts = filterByCompanyContext(
        safeArray(allBankAccounts) as any[],
        companyState.selectedSiteID,
        companyState.selectedSubsiteID
      );
      
      // Filter by date range if provided
      const transactions = dateRange ? contextFilteredTransactions.filter((t: any) => {
        const tDate = safeString(t.date || t.createdAt || t.transactionDate);
      ...
      const profitLoss = { 
        revenue: transactions.filter((t: any) => safeString(t.type) === 'sale').reduce((sum: number, t: any) => sum + safeNumber(t.amount, 0), 0),
        expenses: transactions.filter((t: any) => safeString(t.type) === 'purchase').reduce((sum: number, t: any) => sum + safeNumber(t.amount, 0), 0),
        profit: 0
      };
      ...
      return {
        ...
        cashFlow: cashFlow.map(cf => ({ ...cf, forecast: 0 })), // Add forecast field
  ...
  const getPOSWidgets = async (dateRange?: { startDate: string; endDate: string }): Promise<POSWidgets> => {
    try {
      ...
      // Filter by date range if provided
      const sales = dateRange ? contextFilteredSales.filter((s: any) => {
        const sDate = safeString(s.tradingDate || s.date || s.createdAt);
  ...
  const getHRWidgets = async (dateRange?: { startDate: string; endDate: string }): Promise<HRWidgets> => {
    try {
      ...
      // Filter by company context (site/subsite)
      const employees = filterByCompanyContext(
        (allEmployees || []) as any[],
        companyState.selectedSiteID,
        companyState.selectedSubsiteID
      );
      const timeOffs = filterByCompanyContext(
        (allTimeOffs || []) as any[],
        companyState.selectedSiteID,
        companyState.selectedSubsiteID
      );
  ...
  const getStockWidgets = async (dateRange?: { startDate: string; endDate: string }): Promise<StockWidgets> => {
    ...
    try {
      ...
      const purchases = filterByCompanyContext(
        safeArray(allPurchases) as any[],
        companyState.selectedSiteID,
        companyState.selectedSubsiteID
      );
      const stockCounts = filterByCompanyContext(
        safeArray(allStockCounts) as any[],
        companyState.selectedSiteID,
        companyState.selectedSubsiteID
      );
      ...
      const baseStockValue = Math.max(
        productsWithStock.reduce((sum: number, p: any) => {
          // Use effectiveCost which includes recipe calculations
          return sum + (p.currentStock * p.effectiveCost);
        }, 0),
  ...
  const getCompanyWidgets = async (): Promise<CompanyWidgets> => {
    try {
      ...
      const activeNotifications = notifications?.filter((n: any) => !n.isRead).length || 0;
  ...
  const getMessengerWidgets = async (): Promise<MessengerWidgets> => {
    try {
      ...
      const activeChats = chats?.filter((chat: any) => chat.isActive).length || 0;
      const unreadMessages = chats?.reduce((sum: number, chat: any) => sum + (chat.unreadCount || 0), 0) || 0;
  ...
  const getNotificationWidgets = async (): Promise<NotificationWidgets> => {
    try {
      ...
      const criticalAlerts = notifications?.filter((n: any) => n.priority === 'urgent' || n.priority === 'high').length || 0;
      const systemAlerts = notifications?.filter((n: any) => n.type === 'system').length || 0;
  ...
  const getWidgetData = useCallback(async (dataType: string, options?: { 
    ...
    try {
      ...
      if (dataType.startsWith('stock') || dataType.includes('STOCK')) {
        ...
        return getDataFromWidgets(stockWidgets, dataType, options);
      }
      ...
      if (dataType.startsWith('hr') || dataType.includes('HR') || dataType.includes('EMPLOYEE')) {
        ...
        return getDataFromWidgets(hrWidgets, dataType, options);
      }
