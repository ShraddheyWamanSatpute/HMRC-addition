...
import { encryptEmployeeData, decryptEmployeeData, decryptEmployeeDataArray } from "../utils/EmployeeDataEncryption"
import type {
  Employee,
  Department,
  Role,
...
export const fetchEmployees = async (basePath: string): Promise<Employee[]> => {
  try {
    ...
    // Decrypt sensitive employee data (backward compatible with plain text)
    ...
    return decryptedEmployees as Employee[]
  } catch (error) {
...
export const createEmployee = async (basePath: string, employee: Omit<Employee, "id">): Promise<string | null> => {
  try {
    ...
    // Encrypt sensitive employee data before storing (compliance with HMRC GDPR requirements)
    const encryptedEmployee = await encryptEmployeeData(newEmployee) as Employee
    
    await set(newEmployeeRef, encryptedEmployee)
    
    console.log('[HRs] Employee data encrypted and stored successfully')
    return employeeId
  } catch (error) {
...
export const updateEmployee = async (basePath: string, employeeId: string, updates: Partial<Employee>): Promise<Employee | null> => {
  try {
    ...
    // Encrypt sensitive employee data before storing (compliance with HMRC GDPR requirements)
    const encryptedUpdates = await encryptEmployeeData(updates) as Partial<Employee>
    ...
    if (snapshot.exists()) {
      const employeeData = { id: employeeId, ...snapshot.val() } as Employee
      // Decrypt sensitive fields when returning
      const decryptedEmployee = await decryptEmployeeData(employeeData)
      console.log('[HRs] Employee data encrypted and updated successfully')
