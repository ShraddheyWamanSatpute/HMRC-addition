...
import { ConsentService } from '../gdpr/ConsentService'
import { PrivacyPolicyService } from '../gdpr/PrivacyPolicy'
import {
  FPSSubmissionData,
  EPSSubmissionData,
  EYUSubmissionData,
...
export class HMRCAPIClient {
  ...
  async submitFPS(
    ...
    try {
      ...
      // 2. Check and document lawful basis before processing (GDPR compliance)
      if (userId) {
        const basisCheck = await this.consentService.hasHMRCSubmissionBasis(userId, companyId)
        
        if (!basisCheck.valid) {
          // No valid lawful basis found - document it automatically (Legal Obligation)
          const policyVersion = this.privacyPolicyService.getPrivacyPolicy({
          ...
          try {
            await this.consentService.documentLawfulBasis(
              companyId,
              userId,
              'hmrc_submission',
              'legal_obligation',
            ...
            console.log('[HMRCAPIClient] Lawful basis documented for HMRC submission')
          } catch (docError) {
            console.warn('[HMRCAPIClient] Failed to document lawful basis:', docError)
            // Continue with submission - legal obligation is implicit for HMRC submissions
          }
        } else {
          console.log(`[HMRCAPIClient] Valid lawful basis confirmed: ${basisCheck.basis}`)
        }
      }
      ...
      // 4. Validate XML locally before sending
      ...
      // 5. Get fraud prevention headers (collected client-side)
      const fraudHeaders = this.fraudPreventionService.generateHeaders(userId)

      // 6. Submit via Firebase Functions proxy
      ...
      if (userId) {
        ...
        if (!basisCheck.valid) {
          ...
          try {
            ...
            console.log('[HMRCAPIClient] Lawful basis documented for HMRC EPS submission')
          } catch (docError) {
            console.warn('[HMRCAPIClient] Failed to document lawful basis:', docError)
            // Continue with submission - legal obligation is implicit for HMRC submissions
          }
