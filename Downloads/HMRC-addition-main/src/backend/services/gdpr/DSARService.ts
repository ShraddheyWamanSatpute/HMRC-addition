...
export class DSARService {
  ...
  async getOverdueRequests(companyId: string): Promise<DSARRecord[]> {
    ...
    return requests.filter((r) => {
      // Exclude completed, rejected, or withdrawn requests
      const excludedStatuses: DSARStatus[] = ['completed', 'rejected', 'withdrawn'];
      if (excludedStatuses.includes(r.status)) {
        return false;
      }
  ...
  async getStatistics(companyId: string): Promise<{
    ...
    for (const r of requests) {
      // Status counts - cast to DSARStatus to access extended status values
      const status = r.status as DSARStatus;
      switch (status) {
        case 'pending':
        ...
        case 'rejected':
          ...
          break;
        case 'withdrawn':
          // Withdrawn requests are not counted in stats
          break;
      }
