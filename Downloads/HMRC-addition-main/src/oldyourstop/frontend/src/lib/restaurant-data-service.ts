...
class RestaurantDataService {
  ...
  // Get restaurants with real-time data
  async getRestaurants(filters?: RestaurantSearchFilters): Promise<RestaurantSearchResult> {
    ...
    try {
      ...
      if (comprehensiveData.length > 0) {
        ...
        restaurants = comprehensiveData.map(data => this.transformComprehensiveData(data));
        ...
        restaurants = this.mergeRestaurantData(allRestaurants, [], []);
      } else {
        // Use mock data when no API keys are configured
        console.log('No API keys configured, using mock data');
        restaurants = this.filterMockRestaurants(mockRestaurants, filters);
      }

      const result: RestaurantSearchResult = {
        restaurants: restaurants, // No limit - return all restaurants
        total: restaurants.length,
        filters: filters || {},
        lastUpdated: new Date().toISOString()
      };

      this.setCachedData(cacheKey, result, this.CACHE_TTL.BASIC_INFO);
      return result;
    } catch (error) {
      console.error('Error fetching restaurants:', error);
      // Fallback to mock data on error
      const restaurants = this.filterMockRestaurants(mockRestaurants, filters);
      return {
